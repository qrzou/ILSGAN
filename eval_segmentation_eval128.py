...
class SynSegDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if is_val:
            self.transforms_img = torch_tf.Compose([
                torch_tf.Resize((int(size//2), int(size//2))),
                torch_tf.ToTensor(),
                torch_tf.Normalize(mean=[0.5] * 3, std=[0.5] * 3)])
    ...
    def __getitem__(self, idx):
        ...
        mask_tensor = (self.transforms(mask)[0] > 0.).long() # Bug code? Maybe wrote for the soft mask before.

        # # standard + thresh0.5
        # mask = Image.open(self.maskfiles[idx]).convert("RGB") #
        # mask_tensor = (self.transforms(mask)[0] > 0.5).long() # Bug code? Maybe wrote for the soft mask before.

        # Wrong
        # mask = Image.open(self.maskfiles[idx]).convert("L")
        # mask_tensor = (torch_tf.ToTensor()(mask) > 0).long().squeeze(0)
...
class RealSegTestSet(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.transforms = transforms
        self.img_transforms = torch_tf.Compose([
            torch_tf.Resize(int(scale//2)), torch_tf.CenterCrop(int(scale//2)), torch_tf.ToTensor()])
...
class CubDataset(Dataset):
    def __init__(
        ...
        self.transform = transforms.Compose([
            transforms.Resize((int(size//2 ), int(size//2))),
            transforms.ToTensor(),
...
def calc_metrics_noARIandMSC(metrics, model, testloader, device):
    ...
    for img, gt_mask in testloader:
        ...
        pred_logits = F.interpolate(pred_logits, scale_factor=2.0, mode='bilinear', align_corners=False)
        ...
        # print(img.shape, pred_mask.shape, gt_mask.shape)
...
def segmentation(
    ...
    assert os.path.exists(os.path.join(syn_data, 'img')), os.path.join(syn_data, 'img')
    assert os.path.exists(os.path.join(syn_data, 'mask')), os.path.join(syn_data, 'mask')
    # assert real_data in ['cub', 'flowers', 'lsun_car']

    whole_dataset = SynSegDataset(syn_data, size=64)
    num_samples = len(whole_dataset)
    del whole_dataset

    split_num = int(num_samples * 0.9)

    trainset = SynSegDataset(syn_data, range(split_num), size=64)
    ...
    valset = SynSegDataset(syn_data, range(split_num, num_samples), size=128, is_val=True)
    ...
    resume(dict(model=model, opt=opt, lr_sch=lr_scheduler), os.path.join(run_dir, metric_results[maxiou_idx]['snapshot_pth']))
    ...
    grid_img = [x.to(device) for x in grid_img]
    model.eval().requires_grad_(False)
    grid_pred = [F.interpolate(model(img.to(device)), scale_factor=2.0, mode='bilinear', align_corners=False).max(dim=1)[1] for img in grid_img]
    model.train().requires_grad_(True)
    grid_img = [F.interpolate(x, scale_factor=2.0, mode='bilinear', align_corners=False) for x in grid_img]
...
def main(ctx, dry_run, **config_kwargs):
    ...
    args.run_dir = args.run_dir + '-eval128'
