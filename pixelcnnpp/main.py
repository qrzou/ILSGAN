...
def old_sample(model, device=None):
    """
    Error code.
    :param model:
    :param device:
    ...
    # Old code lacks the require_grad_(False) operation, which leading to CUDA out of memory.
    # model.eval().requires_grad_(False)
...
for epoch in range(args.max_epochs):
    ...
    if (epoch + 1) % args.save_interval == 0:
        ...
        print('sampling...')
        sample_t = sample(model, device)
        sample_t = rescaling_inv(sample_t)
        utils.save_image(sample_t, os.path.join(run_dir, f'synthetic-samples-{epoch:05d}.png'),
                nrow=5, padding=0)
