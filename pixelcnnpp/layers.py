...
class down_shifted_conv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.down_shift(x, pad=nn.ZeroPad2d((0, 0, 1, 0))) if self.shift_output_down else x
...
class down_right_shifted_conv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        x = self.bn(x) if self.norm == 'batch_norm' else x
        return self.right_shift(x, pad=nn.ZeroPad2d((1, 0, 0, 0))) if self.shift_output_right else x
...
class gated_resnet(nn.Module):
    def __init__(self, num_filters, conv_op, nonlinearity=concat_elu, skip_connection=0, conditional=False, h_dim=None):
        ...
        if conditional:
            self.hw = torch.nn.Linear(in_features=h_dim, out_features=2*num_filters, bias=False)
    ...
    def forward(self, og_x, a=None, h=None):
        ...
        if h is not None:
            ...
            x = x + self.hw(h).unsqueeze(-1).unsqueeze(-1)
