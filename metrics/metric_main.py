...
def report_metric(result_dict, run_dir=None, snapshot_pkl=None, sigma=None):  # [NEW]: record the sigma weight in loss
    metric = result_dict['metric']
    assert is_valid_metric(metric)
    if run_dir is not None and snapshot_pkl is not None:
        snapshot_pkl = os.path.relpath(snapshot_pkl, run_dir)

    jsonl_line = json.dumps(dict(result_dict, snapshot_pkl=snapshot_pkl, timestamp=time.time(), sigma=sigma))
    print(jsonl_line)
...
def kid10k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    kid = kernel_inception_distance.compute_kid(opts, max_real=None, num_gen=10000, num_subsets=20, max_subset_size=1000)
